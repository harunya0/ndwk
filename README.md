...
**C# (.NET 10 / Unity):**
```bash
dotnet run --project bindings/csharp/Ndwk.Sample test/ja/ja_033.wav models
```
...
**Go (1.26):**
```bash
cd bindings/go && go run cmd/sample/main.go ../../test/ja/ja_033.wav ../../models
```

**Java (OpenJDK 22+ Panama FFM):**
```bash
javac -d bindings/java/bin bindings/java/src/ndwk/*.java
...
- **多言語SDK対応**: 外部依存ゼロ（0 dependencies）の公式バインディングを同梱：
  - **C# (.NET 10)**: P/Invoke + `NativeLibrary` リゾルバ
...
C言語で提供される `ndwk.h` / `libndwk.so` を通じて、他言語から外部ライブラリ依存ゼロ（0 dependencies）で組み込み可能です。
...
├── bindings/             # 多言語バインディング (0 dependencies)
