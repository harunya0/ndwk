...
SherpaOnnxOfflineRecognizerConfig model_config_create_asr(
    ...
    switch (lang) {
    case NDWK_LANG_JA: { // 日本語 (ReazonSpeech Zipformer)
    ...
    case NDWK_LANG_ZH: { // 中国語 (Paraformer-zh)
    ...
    case NDWK_LANG_EN: { // 英語+欧州 (Parakeet TDT v3)
        ...
        config.model_config.tokens             = tokens_path;
        config.model_config.num_threads        = 4;
        config.model_config.model_type         = "nemo_transducer"; // ★NeMo Transducer
        config.decoding_method                 = "greedy_search";
        config.feat_config.sample_rate         = 16000;
        config.feat_config.feature_dim         = 80;
        break;
    }
    case NDWK_LANG_KO: { // 韓国語 (SenseVoice small)
